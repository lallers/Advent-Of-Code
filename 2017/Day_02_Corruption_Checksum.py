def getSum(input,divisible = False):
    out = 0
    k=0
    for item in range(len(input)):
        item_int = [int(a) for a in input[item].split(",")]
        if divisible == False:
            out += max(item_int) - min(item_int)
        else:
            i=1
            n = len(item_int)
            for x in item_int:
                if i == n: break
                for y in item_int[i:n]:
                    if x % y == 0: 
                        print(x,y,x%y,x/y)
                        out += x/y
                        break
